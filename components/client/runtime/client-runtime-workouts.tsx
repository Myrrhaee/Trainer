...
import { AlertTriangle, ChevronRight, Dumbbell, MessageSquareText, ShieldAlert } from "lucide-react";
...
import type { ClientDemoActor } from "@/components/trainer-os/demo-runtime/types";
...
import { ClientWorkoutFocus } from "./client-workout-focus";
...
export function ClientRuntimeWorkouts({ actorId, assignmentId, sessionId, viewMode }: ClientRuntimeWorkoutsProps) {
  ...
  if (!view || !history) return <SafeWorkoutState actorId={actorId} title="Спортсмен не найден" detail="Проверьте ссылку или вернитесь к началу." />;
  if (view.notFound) return <SafeWorkoutState actorId={actorId} title="Тренировка не найдена" detail="Возможно, тренировка была удалена или ссылка устарела. Данные другого спортсмена не будут показаны." />;
  ...
  function startSession() {
    ...
    router.replace(navigation.href(`/client/workouts?session=${encodeURIComponent(result.receipt.session.session.id)}`));
  }
  ...
  function completeSession() {
    ...
    setCommandError(null);
    router.replace(navigation.href(`/client/workouts?session=${encodeURIComponent(result.receipt.session.session.id)}&view=history`));
  }
  ...
  return (
    <ClientRuntimeShell actorId={actorId} actorName={view.actor.displayName} title="Тренировки" description="План, выполнение и история тренировок.">
      <div className="mb-5 flex gap-2" role="navigation" aria-label="Режим тренировок">
        <Button asChild variant={showHistory ? "outline" : "default"} className="rounded-lg">
          <Link href={navigation.href(`/client/workouts${view.assignment ? `?assignment=${encodeURIComponent(view.assignment.id)}` : ""}`)}>Текущая</Link>
        </Button>
        <Button asChild variant={showHistory ? "default" : "outline"} className="rounded-lg">
          <Link href={navigation.href("/client/workouts?view=history")}>История</Link>
        </Button>
      </div>
      ...
      {showHistory ? (
        ...
        <section className="space-y-5" aria-label="Выполнение тренировки">
          <div className="rounded-lg border border-lime-300/18 bg-lime-300/[0.05] p-5">
            <p className="text-xs uppercase text-lime-200">Тренировка в процессе</p>
            <h2 className="mt-2 text-2xl font-semibold text-zinc-50">{session.sessionTitle}</h2>
            <p className="mt-2 text-sm text-zinc-400">{view.assignment.generalInstruction || "Выполняйте упражнения по порядку и сохраняйте результат каждого подхода."}</p>
          </div>

          <ClientWorkoutFocus actor={actor} sessionId={session.session.id} exercises={session.exerciseLogs} onError={setCommandError} />
          ...
          <div className="sticky bottom-20 z-20 rounded-lg border border-zinc-700 bg-zinc-950/95 p-3 shadow-2xl sm:flex sm:flex-wrap sm:items-center sm:justify-between sm:gap-3 sm:p-4 lg:bottom-4">
            <div className="hidden sm:block">
              <p className="font-medium text-zinc-100">Готовы завершить?</p>
              <p className="text-sm text-zinc-500">Сначала проверьте итоги выполненных и пропущенных подходов.</p>
            </div>
            <Dialog open={completionOpen} onOpenChange={setCompletionOpen}>
              <DialogTrigger asChild><Button className="h-11 w-full rounded-lg bg-lime-200 text-black hover:bg-lime-100 sm:w-auto">Завершить тренировку</Button></DialogTrigger>
...
function HistoryPanel({ actorId, history, onFeedbackViewed }: { actorId: string; history: NonNullable<ReturnType<typeof getClientHistoryView>>; onFeedbackViewed: (sessionId: string) => void }) {
  ...
  return <div className="space-y-4">{history.map((item) => (
    <article key={item.id} className="rounded-lg border border-zinc-800 bg-zinc-950/80 p-4">
      ...
      {item.session ? <Button asChild variant="outline" className="mt-4 rounded-lg"><Link href={navigation.href(`/client/workouts?session=${encodeURIComponent(item.session.session.id)}&view=history`)}>Открыть результат</Link></Button> : null}
    </article>
  ))}</div>;
}
    ...
    <main className="flex min-h-dvh items-center justify-center bg-black px-4 text-center text-zinc-100">
      <div>
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-2">
          {knownActor ? <Button asChild className="rounded-lg bg-lime-200 text-black hover:bg-lime-100"><Link href={navigation.href("/client/me")}>На главную</Link></Button> : null}
          {knownActor ? <Button asChild variant="outline" className="rounded-lg border-zinc-700"><Link href={navigation.href("/client/workouts")}>К назначениям</Link></Button> : null}
