...
Status: accepted target architecture, updated by Backend Foundation B0. PostgreSQL and domain contracts are canonical; ordinary product data access is backend-only.
...
Identity providers and PostgreSQL connectivity are server adapters. Their token, row and API shapes do not become UI or domain contracts.

## Browser reads

- Product browsers do not connect directly to PostgreSQL or a database Data API.
- All authenticated reads use same-origin query services and return Stage 3 read models, not raw rows.
- Query services centralize actor/relation authorization, derived metrics and sensitive-field allowlists.
- Public trainer profile uses an explicit unauthenticated public projection served by the backend.

## Browser writes

- Critical browser writes are prohibited in the target architecture.
- Browser-local drafts/preferences may remain local-only, but durable writes go through backend commands.
...
| SaveWorkoutSessionProgress | API/server action -> repository transaction | Versioned batch upsert logs/signals with authenticated athlete ownership |
...
Server commands authenticate and authorize actor/capability/ownership/relation before repositories run. RLS independently prevents cross-tenant access using transaction-local application actor context. A privileged database role is never authorization.

## Privileged database role boundary
...
Swap PostgreSQL connection/repository implementation, identity verification and media adapter independently. UI contracts and domain commands remain unchanged. Test core migrations and actor-context RLS policies against clean standard PostgreSQL.
...
|---|---|---|
| Browser database access | None in the target architecture; use backend query services | Yes |
