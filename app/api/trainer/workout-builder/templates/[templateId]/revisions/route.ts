...
export async function POST(request: Request, context: { params: Promise<{ templateId: string }> }) {
  ...
  try {
    ...
    const result = await new WorkoutBuilderService().createRevision(actor, templateId, body);
    return NextResponse.json(result, { status: result.outcome === "existing_draft" || result.replay ? 200 : 201 });
  } catch (error) {
